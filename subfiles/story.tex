...
Effective algorithms lead to the definition of a special class of functions. Let $A$ be a countable set, such as $\{0,1\}^\ast$, $\N$, $\Q$, or finite product of these sets. A function $\varphi : A \to A$ is called \textit{computable} if there exists an effective algorithm $E_\varphi$ that, when presented with input $x \in A$, delivers $\varphi(x)$ as an output. Further, for $\xf \in \{0,1\}^\N$, we denote by $\varphi^\xf$ the function defined such that for all $x \in A$, $\varphi^\xf(x)$ is the output of $E_\varphi^\xf$ when presented with input $x$. We say that $\varphi^\xf$ is relatively computable w.r.t. $\xf$.
