...
In this paper, we manipulate infinite binary sequences. Hopefully, algorithmic complexity naturally extends from finite to infinite sequences. For an infinite sequence $\xf \in \{0,1\}^\omega$, we study the behavior of $K[\xf_{1:n}]$ in function of $n \in \N$.
  ...
  The difference of the algorithmic complexity between prefixes of the greedy and lazy binary expansions is well understood. For $s \in [0,1]$, the greedy binary expansion $\xf$ of $s$ and the lazy binary expansion $\xf'$ of $s$ satisfy
	...
	\begin{definition}(Relative Kolmogorov complexity)\label{def:relative kolmogorov complexity}
		We define the Kolmogorov complexity of $x \in \{0,1\}^\ast$ relative to $\beta$ as the length of the shortest binary sequence $x^\ast$ such that there exists a prefix $y$ of the greedy binary expansion of $\beta - 1$ satisfying $\Uref(\langle x^\ast, y \rangle) = x$. It is denoted $K[x|\beta]$.
	\end{definition}
	...
	\begin{definition}
		...
		\end{equation}
		where $\xf'$ is the greedy binary expansion of $s = \delta_\beta(\xf)$.
	\end{definition}
	...
	\begin{theorem}
		...
		for all $\xf \in \{0,1\}^\N$.
	\end{theorem}
	This lower bound is tight, i.e., for every $\beta \in (1,2)$, there exists $\xf \in \{0,1\}^\ast$ such that $\underline\Delta_\beta(\xf) = \bar\Delta_\beta(\xf) = 0$. Our second result is a trivial upper bound, which is mere consequence of the definition of Kolmogorov complexity.
	\begin{theorem}
		Let $\beta \in (1,2)$. Then,
		\begin{equation}
	...
	\end{enumerate}
	$P_\beta$ is called the \textit{minimal polynomial} of $\beta$. We denote by $L_\beta$ the leading coefficient $a_d$ of $P_\beta$ and $T_\beta$ the tail coefficient $a_0$ of $P_\beta$. The roots of $P_\beta$, not including $\beta$, are called the \textit{Galois conjugates of $\beta$} (by root we mean any complex number $z \in \C$ such that $P_\beta(z) = 0$). We denote by $G_\beta$ the set of Galois conjugates of $\beta$. Following Garsia, we the set $G^\ast_\beta = \{z \in G_\beta : |z| > 1 \}$ of Galois conjugates that lay outside of the unit circle. $G^\ast_\beta$ will have a drastic impact of the Kolmogorov complexity of $\beta$-expansions.
	...
	\end{align}
	These multiplicities naturally define an equivalence relationship on $\{0,1\}^n$, by $x \sim_n y \iff \sum_{i=1}^n x_i \beta^{-i} = \sum_{i=1}^n y_i \beta^{-i}$. We denote by $[x]_n$ the equivalence class associated to $\sim_n$. To every $x \in \{0,1\}^n$, we define $M x$ to be the lexicographically maximal element of $[x]_n$. Moreover, given an infinite sequence $\xf$, we define $M \xf$ as being the sequence such that $(M x)_{1:n} := M(x_{1:n})$. The introduction of $M\xf$ allows to derive the main result on the Kolmogorov complexity of the $\beta$-expansions when $\beta$ is algebraic.
	\begin{theorem}
		Let $\beta \in (1,2)$ be an algebraic number. Then,
		\begin{equation}
			0 \leq \underline\Delta_\beta(M\xf) \leq \bar\Delta_\beta(M\xf) \leq \log_\beta\left(L_\beta \prod_{z \in G^\ast_\beta} \left|z \right|\right),
		\end{equation}
		for all $\xf \in \{0,1\}^\N$.
	\end{theorem}
	The above theorem will allow to derive several corollaries. First, we will establish a class of algebraic numbers for which there are no multiplicities, and hence for which $M\xf$ can be replaced by $\xf$ in the above result. This class will be a generalization of the class of \textit{Garsia numbers} introduced in Garsia. Second, we will study another family of algebraic numbers, called \textit{Pisot numbers} Pisot, for which the above equation yields $\underline\Delta_\beta(M\xf) = \bar\Delta_\beta(M\xf) = 0$. In the next section, we will exploit this property to modify slightly the A/D converter of Daubechies in order to control the Kolmogorov complexity of the $\beta$-expansion it delivers.

	We proceed to introduce a class of algebraic numbers, inspired and generalized from the Garsia numbers introduced in Garsia. First, we restrict the class of algebraic numbers as follows. A real number $\beta$ is said to be an \textit{algebraic integer} if $\beta$ is an algebraic number, and that the leading coefficient $L_\beta$ of its minimal polynomial $P_\beta$ satisfies $L_\beta = 1$. Further, we define the subset $\mathscr C$ of algebraic integers as follows. An algebraic integer $\beta$ belongs to $\mathscr C$ if and only if the tail coefficient $T_\beta$ of its minimal polynomial $P_\beta$ satisfies $T_\beta \geq 2$. The class $\mathscr C$ yields a very important property: for $\beta \in \mathscr C$, there are no multiplicities, i.e., $M \xf = \xf$ for all $\xf \in \{0,1\}^\N$. This results in the following corollary.
	\begin{corollary}
	...
	\end{equation}
	for all $\xf \in \{0,1\}^\N$. More generally, the $m$-th root of any integer $k \geq 2$ belongs to $\mathscr C$, since its minimal polynomial is $P_{\sqrt[m]{k}} = X^m - k$. The Galois conjugates of $\sqrt[m]{k}$ are $\sqrt[m]{k} \exp\left(i \frac{2\pi \ell}{m}\right)$, $\ell \in 1, \ldots, m - 1$. This yields
	\begin{align}
...
\begin{enumerate}
	\item Defining the complexities wrt beta

	\item Defining the classes of words defining the same beta-numbers
	\item Defining the lexicographically largest word like that
	...
	\item Corollary for algebraic numbers with roots outside of the unit circle
	\item Corollary for greedy expansions
	
	Miscellaneous theorems:
	\item Theorem for almost all beta in 1, racine de 2
	\item Theorem for almost all beta in 1.497, 2.
\end{enumerate}
