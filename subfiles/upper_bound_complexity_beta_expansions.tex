...
\begin{proof}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \begin{equation}\label{eq:1:proof:cor:upper bound on the relative compressibility of beta expansions for algebraic beta}
           m_n \#\hat  F(\yf_{1:n}) \leq |J(\yf_{1:n})|^{(n)} \overset{(\ref{eq:definition of superset of size 2 -n more})}{=} |J(\yf_{1:n})| + 2\cdot 2^{-n} \overset{(\ref{eq:J x:def:multivalued function converting from base 2 to base beta})}{=} 4 2^{-n}.
        \end{equation}
    ...
    \begin{equation}
       \#F(\yf_{1:n}) \leq \# \hat F(\yf_{1:n}) \leq \frac{\displaystyle  2(n\log_\beta(2))^{k_\beta}\left(L_\beta \Pi_\beta^+ \right)^{n\log_\beta(2)}}{L_\beta \Pi_\beta}\cdot
    \end{equation}
\end{proof}
\noindent We further show that $M_\beta$ is computable.
...
\begin{proof}
    ...
    \end{equation}
    By Corollary \ref{cor:M is computable for algebraic beta}, $M_\beta$ is computable, so $F := M_\beta \circ f_{2 \to \beta}$ is computable relatively to $\beta$. By Theorem \ref{thm:computable multivalued functions and complexity}, this yields
    \begin{align}
        K[M_\beta\xf_{1:n\log_\beta(2)}]\leq K[\yf_{1:n}] + \log \# F(\yf_{1:n}) + \log\log \# F(\yf_{1:n}) + \bigOx[1]{n}.
    \end{align}
    ...
    \begin{equation}
        \# (F(\yf_{1:n})) \leq \frac{2}{L_\beta\Pi_\beta}  (n\log_\beta(2))^{k_\beta}\left(L_\beta \Pi_\beta^+ \right)^{n\log_\beta(2)}\cdot
    \end{equation}
    Note that then,
    \begin{equation}
