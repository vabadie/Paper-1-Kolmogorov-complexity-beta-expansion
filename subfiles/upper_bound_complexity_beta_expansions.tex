...
\section{Upper bounds on the complexity of $\beta$-expansions}
...
\begin{equation}\label{eq:main:def:multivalued function converting from base 2 to base beta}
    f_{2 \to \beta}(x) := \left\{ y \in \{0,1\}^{\len x \log_\beta(2)} :  \sum_{i=1}^{\len y} y_i \beta^{-i} \in J(x)\right\},
\end{equation}
...
\end{equation}
\end{definition}
\noindent By the same arguments that lead to the definition of $f_{\beta \to 2}$, one can establish that
...
\begin{equation}\label{eq:1:introduction to the formalism of Bernoulli convolution}
    \delta_{\sum_{i=1}^{\len y} y_i \beta^{-i}}(J(x)) = \begin{cases}
        1 \text{ if } \sum_{i=1}^{\len y} y_i \beta^{-i} \in J(x),\\
        0 \text{ if } \sum_{i=1}^{\len y} y_i \beta^{-i} \notin J(x),
    \end{cases}
\end{equation}
By summing over all $y \in \{0,1\}^{n\log_\beta(2)}$, and by definition of $f(x)$ in (\ref{eq:main:def:multivalued function converting from base 2 to base beta}), we get
\begin{equation}\label{eq:2:introduction to the formalism of Bernoulli convolution}
    \sum_{y \in \{0,1\}^{n\log_\beta(2)}}\delta_{\sum_{i=1}^{\len y} y_i \beta^{-i}}(J(x)) = \#f(x).
\end{equation}
This framework matches exactly that of the Bernoulli convolution, introduced in \cite[Section 6]{jessenDistributionFunctionsRiemann}. Define $I_\beta := \left[0, \frac{1}{\beta-1} \right]$, $C(I_\beta)$ to be the set of continuous functions $f : I_\beta \to \R$, and $\MM$ to be the set of regular Borel measures on $I_\beta$ (see Appendix). Consider the sequence of regular Borel measures $(\nu_{\beta,m})_{m \in \N}$ defined by 
...
\end{equation}
The fact that there is indeed such a regular Borel measure $\mu$ is a standard result of measure theory, see Appendix for greater details. The Bernoulli convolution has been widely studied in the literature. In particular, it was shown that that for $\lambda$-almost all $\beta \in (1,2)$, $\nu_\beta$ is absolutely continuous with respect to the Lebesgue measure $\lambda$, i.e., for all $A \in \BB$, $\lambda(A) = 0 \Rightarrow \nu_\beta(A) = 0$. In particular, the Radon-Nykodym theorem \cite[Section 31, Theorem B]{halmos1950MeasureTheory} states that if a measure $\nu$ is absolutely continuous with respect to the Lebesgue measure $\lambda$, there exists a function $h_\beta : I_\beta \to \R$, called \textit{Radon-Nikodym derivative of $\nu_\beta$}, such that 
\begin{equation}
    \nu_\beta\left([a,b]\right) = \int_{a}^{b} h_\beta(x) dx,
\end{equation}
for all $a, b \in I_\beta$, $a < b$. We can exploit this to study the asymptotic behavior of $\#f(x)$.
\begin{theorem}\label{thm:asymptotic behavior of card f x when the Bernoulli convolution is absolutely continuous}
    ...
    \begin{equation}
        \limsup_{n\to\infty} 2^{-n\log_\beta(2/\beta)} \#f(\yf_{1:n}) \leq \left(2 + \frac{1}{\beta-1}\right) h_\beta\left(\sum_{i=1}^\infty \yf_i 2^{-i}\right),
    \end{equation}
    for all $\yf \in \{0,1\}^\N$.
\end{theorem}
\begin{proof}
    The proof follows that of \cite[Lemma 3.4]{kempton2013counting}. Let $\beta \in (1,2)$ such that $\nu_\beta$ is absolutely continuous with respect to $\lambda$, let $h_\beta : I_\beta \to \R$ be the Radon-Nikodym derivative of $\nu_\beta$, and fix $\yf \in \{0,1\}^\N$. For $n \in \N$, define $a_n$ and $b_n$ so that $J(\yf_{1:n}) = [a_n,b_n]$. Fix $n \in \N$, and define, for all $m \geq n\log_\beta(2)$, then set 
    \begin{equation}
        A_{m,n} := \{ z \in \{0,1\}^{m} : z_{1:n\log_\beta(2)} \in f(\yf_{1:n}) \}.
    \end{equation}
    We make the following two remarks:
    \begin{enumerate}[label=(\alph*)]
        \item For all $y \in f(\yf_{1:n})$, there are $2^{m-n\log_\beta(2)}$ elements in $A_{m,n}$ that are suffixes of $y$. This implies that
        \begin{equation}
            \# A_{m,n} \geq 2^{m - n\log_\beta(2)} \# f(\yf_{1:n}).
        \end{equation}
        \item For every $z \in A_{m,n}$, since $z_{1:n\log_\beta(2)} \in f(\yf_{1:n})$, we have that
        \begin{equation}
            a_n \leq \sum_{i=1}^{n\log_\beta(2)} z_i \beta^{-i} \leq \sum_{i=1}^{m} z_i \beta^{-i} \leq \sum_{i=1}^{n\log_\beta(2)} z_i \beta^{-i} + \frac{\beta^{-n\log_\beta(2)}}{\beta-1} \leq b_n + \frac{\beta^{-n\log_\beta(2)}}{\beta-1}.
        \end{equation}
        Hence, for every $z \in A_{m,n}$,
        \begin{equation}
            \sum_{i=1}^{m} z_i \beta^{-i} \in \left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right],
        \end{equation}
        which translates to
        \begin{equation}
            \#A_{m,n} \leq 2^m\nu_{\beta,m}\left(\left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right]\right).
        \end{equation}
    \end{enumerate}
    Combining these two remarks, we get that 
    \begin{equation}
        2^{-n\log_\beta(2)} \# f(\yf_{1:n}) \leq \nu_{\beta,m}\left(\left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right]\right).
    \end{equation}
    By taking the limit superior when $m \to \infty$, Lemma Appendix (bounding the absolute continuous limit measure, use a continuous function that gets closer and closer to the point) delivers
    \begin{equation}\label{eq:0:proof:thm:asymptotic behavior of card f x when the Bernoulli convolution is absolutely continuous}
        2^{-n\log_\beta(2)} \# f(\yf_{1:n}) \leq \nu_{\beta}\left(\left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right]\right).
    \end{equation}
    ...
    \begin{equation}\label{eq:1:proof:thm:asymptotic behavior of card f x when the Bernoulli convolution is absolutely continuous}
        \nu_\beta\left(\left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right]\right) = \int_{a_n}^{b_n + \frac{2^{-n}}{\beta-1}} h_\beta(x) dx.
    \end{equation}
    ...
    \begin{align}
        \lim_{n \to \infty} \frac{\nu_\beta\left(\left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right]\right)}{ 2^{-n}\left(2+\frac{1}{\beta-1}\right)}& = \lim_{n \to \infty} \frac{\nu_\beta\left(\left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right]\right)}{b_n + \frac{2^{-n}}{\beta-1} - a_n}\\
        & = \lim_{n \to \infty} \frac{\int_{a_n}^{b_n + \frac{2^{-n}}{\beta-1}} h_\beta(x) dx}{b_n + \frac{2^{-n}}{\beta-1} - a_n}\\
    ...
    \begin{align}
        \limsup_{n\to \infty}2^{-n(\log_\beta(2)-1)} \# f(\yf_{1:n}) &\leq  \limsup_{n \to \infty}\frac{\nu_\beta\left(\left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right]\right)}{ 2^{-n}}\\
        &= \lim_{n \to \infty}\frac{\nu_\beta\left(\left[a_n, b_n + \frac{2^{-n}}{\beta-1}\right]\right)}{ 2^{-n}}\\
        &= \left(2+ \frac{1}{\beta-1}\right)h_\beta\left( \sum_{i=1}^\infty \yf_i \beta^{-i}\right).
    \end{align}
\end{proof}
As a direct Corollary, we get Theorem \ref{thm:upper bound on the relative compressibility of beta expansions for almost all beta in 1 sqrt 2}.
...
\begin{proof}
    ...
    \begin{equation}
        2^{-n\log_\beta(2/\beta)} \#f(\yf_{1:n}) \leq \left(2 + \frac{1}{\beta-1}\right) h_\beta\left(\sum_{i=1}^\infty \yf_i 2^{-i}\right) + 1 =: C,
    \end{equation}
    so 
    \begin{equation}
        \#f(\yf_{1:n}) \leq C \cdot 2^{n\log_\beta(2/\beta)},
    \end{equation}
    for large enough $n \in \N$. This results in 
    \begin{align}
        &\limsup_{n \to \infty} \frac{\log\#(f_{2 \to \beta}(\yf_{1:n}))}{n} = \log_\beta(2/\beta)\label{eq:log f:cor:upper bound on the relative compressibility of beta expansions for almost all beta in 1 2}\\
...
\subsection{Upper bound when $\beta$ is algebraic}
...
\begin{lemma}\label{lem:generalization of the garsia separation lemma}
    ...
    \end{equation}
\end{lemma}
As this result is tied to algebraic considerations that deviate a lot from the message of this paper, we postpone the proof to the Appendix. \textit{This Lemma has two important corollaries, that are essential to establish \ref{thm:upper bound on the relative compressibility of beta expansions for algebraic beta}. First, this implies that we can lower bound the distance between numbers of the form $\sum_{i=1}^n (Mx)_i \beta^{-i}$, $x \in \{0,1\}^n$, and second, we can show that $M_\beta$ is computable.}
...
\begin{lemma}\label{lem:bounding the size of the output set of the multivalued function projected on lexicographically maximal sequences}
    ...
    \begin{equation}\label{eq:main:lem:bounding the size of the output set of the multivalued function projected on lexicographically maximal sequences}
        \# (M_\beta \circ f_{2 \to \beta}(\yf_{1:n})) \leq \frac{ 2}{L_\beta\Pi_\beta}(n\log_\beta(2))^{k_\beta}\left(L_\beta \Pi_\beta^+ \right)^{n\log_\beta(2)}\cdot
    \end{equation}
\end{lemma}
\begin{proof}
    Define $F_\beta := M_\beta \circ f_{2 \to \beta}$. Let $\beta \in (1,2)$ be an algebraic number and $n \in \N$. We will proceed to find an upper bound for $\# F_\beta(\yf_{1:n})$. 
    \begin{enumerate}[label=(\alph*)]
        \item First note that $F_\beta(\yf_{1:n}) \subseteq f_{2 \to \beta}(\yf_{1:n})$. Indeed, let $x \in F_\beta(\yf_{1:n})$. Then, there exists $u \in f_{2 \to \beta}(\yf_{1:n})$ such that $M_\beta(u) = x$. Since $M_\beta^{-1}(\{x\}) = [x]_\beta$, then we deduce that $u \sim_\beta x$, i.e.,
        \begin{equation}\label{eq:0:proof:cor:upper bound on the relative compressibility of beta expansions for algebraic beta}
            \sum_{i=1}^{n\log_\beta(2)} x_i \beta^{-i} = \sum_{i=1}^{n\log_\beta(2)} u_i \beta^{-i} \overset{(a)}{\in} J(\yf_{1:n}),
        \end{equation}
        where (a) follows from Definition \ref{def:multivalued function converting from base 2 to base beta} of $f_{2 \to \beta}$. We conclude that, indeed, $x \in f_{2 \to \beta}(\yf_{1:n})$, so $F_\beta(\yf_{1:n}) \subseteq f_{2 \to \beta}(\yf_{1:n})$.
        \item As a direct consequence, remark that by denoting $m_n$ to be the smallest distance between two different numbers of the form $\sum_{i=1}^{n\log_\beta(2)} x_i \beta^{-i}$, $x \in F_\beta(\yf_{1:n})$, we have that 
        \begin{equation}\label{eq:1:proof:cor:upper bound on the relative compressibility of beta expansions for algebraic beta}
           m_n \# F_\beta(\yf_{1:n}) \leq |J(\yf_{1:n})| = 2\cdot 2^{-n}.
        \end{equation}
        \item Finally, since $f_{2\to \beta}(\yf_{1:n}) \subseteq \{0,1\}^{n \log_\beta(2)}$, then $F_\beta(\yf_{1:n}) \subseteq M_\beta(\{0,1\}^{n\log_\beta(2)})$. By Corollary \ref{cor: separation lemma for lexicographically maximal sequences}, this shows that 
    ...
    \begin{equation}
        \# F_\beta(\yf_{1:n}) \leq \frac{\displaystyle  2(n\log_\beta(2))^{k_\beta}\left(L_\beta \Pi_\beta^+ \right)^{n\log_\beta(2)}}{L_\beta \Pi_\beta}\cdot
    \end{equation}
\end{proof}
We further show that $M_\beta$ is computable.
...
\begin{proof}
    ...
    Now, fix $\beta \in (1,2)$ algebraic, and $x \in \{0,1\}^\ast$. We define $n := |x|$. Let $P_\beta$ be the minimal polynomial of $\beta$ of leading coefficient $L_\beta$, of degree $d \in \N$, and denote by $\alpha_1 < \alpha_2 < \ldots < \alpha_k$ its real roots, for some $k \leq d$. Note that $\beta = \alpha_{\ell_\beta}$ for some $\ell_\beta \in \{1,\ldots, k\}$. Recall the definition of $\Pi_\beta$, $\Pi_\beta^+$ and $k_\beta$ in (\ref{eq:definition of pi beta - pi beta + and k beta}). Define two rational numbers $q,q_+$ such that $0 < q \leq L_\beta \Pi_\beta$ and $q_+ \geq \beta L_\beta \Pi_\beta^+$. We construct an algorithm that computes $x \mapsto \langle [x]_\beta \rangle$.
    \begin{algorithm}[ht]
...
\begin{proof}
    ...
    \end{equation}
    By Corollary \ref{cor:M is computable for algebraic beta}, $M_\beta$ is computable, so $F_\beta := M_\beta \circ f_{2 \to \beta}$ is computable relatively to $\beta$. By Theorem \ref{thm:computable multivalued functions and complexity}, this yields
    \begin{align}
        K[M_\beta\xf_{1:n\log_\beta(2)}]\leq K[\yf_{1:n}] + \log \# F_\beta(\yf_{1:n}) + \log\log \# F_\beta(\yf_{1:n}) + \bigOx[1]{n}.
    \end{align}
    ...
    \begin{equation}
        \# (F_\beta(\yf_{1:n})) \leq \frac{2}{L_\beta\Pi_\beta}  (n\log_\beta(2))^{k_\beta}\left(L_\beta \Pi_\beta^+ \right)^{n\log_\beta(2)}\cdot
    \end{equation}
    Note that then,
    \begin{equation}
        \limsup_{n \to \infty} \frac{\log_2\# (F_\beta(\yf_{1:n}))}{n} \leq \log_\beta\left(L_\beta \Pi_\beta^+\right),
    \end{equation}
    and 
    \begin{equation}
        \limsup_{n \to \infty} \frac{\log_2 \log_2\# (F_\beta(\yf_{1:n}))}{n} = 0.
    \end{equation}
    We conclude that
    \begin{equation}
        \bar\Delta_\beta(M_\beta\xf) = \limsup_{n\to \infty} \frac{K[M_\beta\xf_{1:n\log_\beta(2)}] - K[\yf_{1:n}]}{n} \leq \log_\beta\left(L_\beta \Pi_\beta^+\right).
    \end{equation}
\end{proof}
